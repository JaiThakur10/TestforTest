...
import { FaEye, FaInfinity, FaShoppingCart, FaTrademark } from "react-icons/fa";
            ...
            <tbody>
              {data.map((item, index) => (
                <tr key={item.rank} className={`${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} border-b hover:bg-gray-100 transition duration-150`}>
                  ...
                  <td className="py-4 px-6 text-center"><FaInfinity className=" text-green-500"/>{item.calmarRatio.toFixed(2)}</td>
